class WordCloudData {
	...
	populateWordsToCounts(inputString) {
		...
		let stringAsArray = inputString.split(/[().?!:,]+\s?|\s\-\s|\s/g)
		stringAsArray.forEach((word) => {
			if (word != "") {
				let upperCaseWord =
					word[0].toUpperCase() + word.substring(1)
				let matchedWord = word
				let matchedWordValue =
					this.wordsToCounts.get(word) ||
					this.wordsToCounts.get(upperCaseWord)

				if (this.wordsToCounts.get(upperCaseWord)) {
					this.wordsToCounts.delete(upperCaseWord)
				}

				let nextCount = matchedWordValue + 1 || 1
				this.wordsToCounts.set(word, nextCount)
			}
		})
